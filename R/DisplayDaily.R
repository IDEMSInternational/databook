#' Display Daily Meteorological Data
#'
#' @description
#' Display daily meteorological data in a readable table format.
#' This function was developed by Helen Greatrex as part of the SSC-RCLIM package. It formats daily meteorological data for easy viewing, displaying values for each day of the month and summarising monthly statistics. The function is similar in display format to the INSTAT "DisplayDaily" feature.
#'
#' @param Datain A data frame containing meteorological data, with one column for each variable, one row for each date, and the following columns (case-sensitive):
...
#' @param Stations A character vector of station names to view (e.g., `"KUND0002"`, `c("Paris", "London")`). If missing, all stations in `Datain` are selected.
#' @param Variables A character vector of column names for the variables to display (e.g., `"Rain"`, `"TMin"`, `"RH"`). If missing, all variables except metadata columns are selected.
#' @param option An integer (1 or 2) indicating date format: 
#'   - `1`: `Date` column format (yyyy-mm-dd)
#'   - `2`: separate `Year`, `Month`, and `Day` columns.
#' @param Years An integer vector of years to view (default is all years in `Datain`).
#' @param Misscode Character string representing the display value for missing data (e.g., "-").
#' @param Tracecode Character string for trace amounts of rainfall (default "tr").
#' @param Zerocode Character string for zero values (default "--").
#' @param Fileout Optional file path for saving the output table. If `NA`, results are printed to the console.
#' @param monstats A character vector of monthly statistics to compute, options include `"min"`, `"mean"`, `"median"`, `"max"`, `"IQR"`, and `"sum"`.
#'
#' @return This function prints a formatted daily table for each specified station, variable, and year. If a file path is specified in `Fileout`, the output is saved to that file.
#'
#' @details
